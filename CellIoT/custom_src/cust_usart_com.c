...
SQNS_STATUS CELLIOT_USART_Ressources_Init(void)
{
	SQNS_STATUS status = SQNS_SUCCESS;

//	do
//	{
//		status = CELLIOT_USART_Context_Init();
...
//		}
//		if(SQNS_SUCCESS != status)
//		status = CELLIOT_USART_Handler_Init();
//
//	}while(0);
...
SQNS_STATUS CELLIOT_USART_Context_Init(void)
{
	SQNS_STATUS status = SQNS_SUCCESS;
	...
	do
	{
		...
		if (NULL == (GET_DRIVER_CXT(p_Global_Context)->q_txQueue = xQueueCreate((UBaseType_t)CELLIOTSHIELD_USART_QUEUE_SIZE, (UBaseType_t)CELLIOTSHIELD_USART_BUFFER_SIZE)))
		{
			vSemaphoreDelete(GET_DRIVER_CXT(p_Global_Context)->sendSemaphore);
		...
		if (NULL == (GET_DRIVER_CXT(p_Global_Context)->q_rxQueue = xQueueCreate((UBaseType_t)CELLIOTSHIELD_USART_QUEUE_SIZE, (UBaseType_t)CELLIOTSHIELD_USART_BUFFER_SIZE)))
		{
			vSemaphoreDelete(GET_DRIVER_CXT(p_Global_Context)->sendSemaphore);
			vSemaphoreDelete(GET_DRIVER_CXT(p_Global_Context)->receiveSemaphore);
		...
		//GET_DRIVER_CXT(p_Global_Context)->p_UsartHandle = &CELLIOTSHIELD_USART_RTOS_HANDLE;
...
SQNS_STATUS CELLIOT_USART_Handler_Init(void)
{
	BaseType_t status;
...
//			while (kStatus_Success!= USART_RTOS_Send(&CELLIOTSHIELD_USART_RTOS_HANDLE, GET_DRIVER_CXT(p_Global_Context)->sendXfer.data, GET_DRIVER_CXT(p_Global_Context)->sendXfer.dataSize))
//			{}
...
//		if(pdPASS == xStatus)
//		{
//			//USART_RTOS_Receive(&CELLIOTSHIELD_USART_RTOS_HANDLE, CELLIOTSHIELD_USART_RX_BUFFER, 2, &n);
//		}
//		if(RTOS_USART_SEND_PENDING == ( ev & RTOS_USART_SEND_PENDING) )
//		{
//			xSemaphoreTake( GET_DRIVER_CXT(p_Global_Context)->receiveSemaphore, 0);
//			while (kStatus_Success!= USART_RTOS_Receive(&CELLIOTSHIELD_USART_RTOS_HANDLE, GET_DRIVER_CXT(p_Global_Context)->sendXfer.data, GET_DRIVER_CXT(p_Global_Context)->sendXfer.dataSize))
//			{}
//			xSemaphoreGive( GET_DRIVER_CXT(p_Global_Context)->receiveSemaphore );
//		}
		vTaskDelay(xTicksToWaitForEvent);
	}
	USART_RTOS_Deinit(&CELLIOTSHIELD_USART_RTOS_HANDLE);
	CELLIOT_USART_Context_Deinit();
	vTaskDelete( NULL );
}

SQNS_STATUS CELLIOT_USART_Context_Deinit(void)
{
	vSemaphoreDelete(GET_DRIVER_CXT(p_Global_Context)->sendSemaphore);
	vSemaphoreDelete(GET_DRIVER_CXT(p_Global_Context)->receiveSemaphore);
	vQueueDelete(GET_DRIVER_CXT(p_Global_Context)->q_txQueue);
