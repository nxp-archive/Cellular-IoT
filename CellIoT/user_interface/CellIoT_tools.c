...
{
	return CellIoT_tools_setSocketCfgExt(gsm.m.conn_val_id + 1, 1, 1, 0, 1, 1, NULL, NULL, 1);
}

int32_t SOCKETS_SetCfg(void)
{
#ifdef USE_TRUPHONE
	return CellIoT_tools_setSocketCfg(gsm.m.conn_val_id + 1, 1, 1450, 0, 600, 50, 1);
#else
	/* PDP=3 is used for Verizon network! */
	return CellIoT_tools_setSocketCfg(gsm.m.conn_val_id + 1, 3, 1450, 0, 600, 50, 1);
#endif
}

int32_t SOCKETS_SetSockSecurity(void)
{
    return CellIoT_tools_setSocketSecurity(gsm.m.conn_val_id + 1, gsm.m.conn_val_id + 1, 0, NULL, NULL, 1); // taylan
}

int32_t SOCKETS_SetTLSSecurityCfg(void)
{
	return CellIoT_tools_setTLSSecurityProfileCfg(gsm.m.conn_val_id + 1, 2, "0x3C", 0, 0, 0, 0, NULL, NULL, NULL, 1);
}
 ...
 */
gsmr_t
CellIoT_tools_WriteCertKeyInNVM(const char * certkey, SQNS_MQTT_CERTORKEY type, uint8_t index, size_t certkeysize, const gsm_api_cmd_evt_fn evt_fn, void* const evt_arg, const uint32_t blocking)
{
 ...
 */
gsmr_t
CellIoT_tools_getHostIP(const char * hostName, const gsm_api_cmd_evt_fn evt_fn, void* const evt_arg, const uint32_t blocking)
{
    GSM_MSG_VAR_DEFINE(msg);
 ...
 */
gsmr_t
CellIoT_tools_socketDial(	uint8_t connId,
						uint8_t txProt,
						uint16_t rHostPort,
						const char* ip,
						uint8_t closureType,
 ...
 */
gsmr_t
CellIoT_tools_socketSend( uint8_t connId, const unsigned char * pTX , uint32_t sTx )
{
    GSM_MSG_VAR_DEFINE(msg);
 ...
 */
uint32_t
CellIoT_tools_socketRecv( uint8_t connId )
{
	uint8_t ret =  gsmERR;
 ...
 */
uint32_t
CellIoT_tools_socketReadData( uint8_t connId, unsigned char * pRX , uint16_t rcvlen )
{
	uint32_t ret = 0;
	...
	{
		(uint32_t) CellIoT_tools_socketRecv(connId);
...
#include "task.h"
void
CellIoT_tools_socketStoreRXData_Pending_Info( uint32_t connId, uint32_t BP )
{
	taskENTER_CRITICAL();
 ...
 */
gsmr_t
CellIoT_tools_setSocketSecurity(uint8_t spId, uint8_t connId, uint8_t enable, const gsm_api_cmd_evt_fn evt_fn, void* const evt_arg, const uint32_t blocking)
{
    GSM_MSG_VAR_DEFINE(msg);
 ...
 */
gsmr_t
CellIoT_tools_setTLSSecurityProfileCfg(	uint8_t spId,
									uint8_t version,
 ...
 */
gsmr_t
CellIoT_tools_setSocketCfgExt(uint8_t connId, uint8_t srMode, uint8_t recvDataMode, uint8_t keepalive, uint8_t listenAutoRsp, uint8_t sendDataMode, const gsm_api_cmd_evt_fn evt_fn, void* const evt_arg, const uint32_t blocking)
{
    GSM_MSG_VAR_DEFINE(msg);
 ...
 */
gsmr_t
CellIoT_tools_setSocketCfg(uint8_t connId, uint8_t cid, uint16_t pktsize, uint16_t maxto, uint32_t connto , uint32_t txTo , const uint32_t blocking)
{
    GSM_MSG_VAR_DEFINE(msg);
 ...
 */
gsmr_t
CellIoT_tools_socketClose(uint32_t connId)
{
    GSM_MSG_VAR_DEFINE(msg);
...
gsmr_t
CellIoT_tools_setLogInModule(void)
{
	GSM_MSG_VAR_DEFINE(msg);
 ...
 */
gsmr_t
CellIoT_tools_readConfTestMode(void)
{
    GSM_MSG_VAR_DEFINE(msg);
 ...
 */
gsmr_t
CellIoT_tools_setConfTestMode(const char * ctm, const gsm_api_cmd_evt_fn evt_fn, void* const evt_arg, const uint32_t blocking)
{
	GSM_MSG_VAR_DEFINE(msg);
